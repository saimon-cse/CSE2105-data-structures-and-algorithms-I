...
void Graph::floydWarshall()
{
    ...
    for(k=1; k<nVertices; k++)
    {
        for(i=1; i<nVertices; i++)
        {
            ...
            {
                if(i!=j && distanceMatrix[i][k]!=INT_MAX && distanceMatrix[k][j]!=INT_MAX)
                {
...
int main()
{
    ...
    while(1)
    {
        cout<<"1. Clear the values of distance and parent matrices."<<endl;
        cout<<"2. Implement Floyd-Warshall Algorithm."<<endl;
        cout<<"3. Implement Johnson's Algorithm"<<endl;
        cout<<"4. Print the shortest path from a vertex to another vertex."<<endl;
